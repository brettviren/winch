...
*winch* requires ~podman~, Python 3 with the Click and Neworkx packages for most basic use.
...
#+begin_example
$ winch -c winch.toml build -i image=debian-bookworm-edit
$ winch -c winch.toml build -i all
#+end_example

Here, we select an instance with ~-i image=debian-bookworm-edit~ that provides a
selector.  One may instead give a hash shown from ~list~ as ~-i <hash>~.

The ~-d/--deps~ is an important alternative to ~-i/--image~ when there are many
...
- The user may tell *winch* to use different parameters that serve the role of
  ~image~ and ~containerfile~ via command line arguments.

- The ~Containerfile~ template may include a ~COPY~ to add files listed in ~files~ to the image.
...
*winch* itself does not rely on any particular environment settings.  However,
users may benefit from setting some of these variables:
